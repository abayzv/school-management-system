...
class assignFeeClassRoom extends React.Component {
    ...
    action = (cell, row, rowIndex, formatExtraData) => {
        if (row.status == "Asigned") {
            return <button disabled class="btn btn-dark">Assign</button>
        } else {
            return <button onClick={() => {
                this.assignStudent(row.id)
    ...
    getStudentClassRoom = () => {
        axios.get(`https://anifox.posapp.id/api/searchclassroom?class=${this.state.search.classId}&section=${this.state.search.sectionId}&feegroupid=${this.props.match.params.id}}`).then((result) => {
...
export default assignFeeClassRoom